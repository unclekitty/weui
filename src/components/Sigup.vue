<template>
  <div class="page-sigup">
     <box gap="15px 15px">
      ...
      <section v-if="step==1">
        <group class="form-group">
          <x-input title="密码" type="password" placeholder="请输入密码(6～16位)" v-model="password" :min="6" :max="16"></x-input>
        </group>
        <group class="form-group">
          <x-input title="确认密码" v-model="repassword" type="password" placeholder="请确认密码" :equal-with="password" :min="6" :max="16"></x-input>
        </group>
        <div class="wp">
          <span>
            继续操作视为同意<router-link class="link" to="sigup">《用户注册协议》</router-link>
